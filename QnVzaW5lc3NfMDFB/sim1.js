...
function TimeController(){
  A.money-=A.business.employees.total();
  let m=A.business.employees.morale;
...
EmployeeTaxDetermine() {
    const hccpe = [.04, .07, .1]; // Healthcare cost per difficulty (easiest is highest index)
    const dV = this.dV;
    ...
    for (let i = 0; i < groups.length; i++) {
        ...
        let living = 1775.5 + ((maxDV - dV) * 100);
        ...
        if (grossPay <= 0) {
            tax = 0.10 + (0.05 * (maxDV - dV));       // Higher tax on harder difficulty
            retirement = 0.05 + (0.05 * (maxDV - dV)); // Higher retirement on harder difficulty
 ...
 EmployeeMoralCalc(){
  ...
  for(let i=0;i<pay.length;i++){
    pay[i];
    if(pay[i]<0){
      m-=Math.abs(pay[i]) * (i + 1) * (0.002 * (4 - this.dV));
    }if(pay[i]>Math.abs(7*i+1)*parseFloat(XXX[i][2])){
       m+= (pay[i] - Math.abs(7*i+1)*parseFloat(XXX[i][2])) * Math.abs(7 * i + 1) * (0.0015 * (4 - this.dV));
    }
  }
  return m;
}
